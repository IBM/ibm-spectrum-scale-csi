...
type SpectrumScaleConnector interface {
	...
	SetBucketKeys(ctx context.Context, opts map[string]interface{}, access map[string]string) error
	CreateCosFileset(ctx context.Context, filesystemName string, filesetName string, opts map[string]interface{}, access map[string]string) error
