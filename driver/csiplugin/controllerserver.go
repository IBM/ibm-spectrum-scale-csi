...
func (cs *ScaleControllerServer) createLWVol(ctx context.Context, scVol *scaleVolume) (string, error) {
	logger.Infof(ctx, "volume: [%v] - ControllerServer:createLWVol", scVol.VolName)
	var err error

	// check if directory exist
	dirExists, err := scVol.PrimaryConnector.CheckIfFileDirPresent(scVol.VolBackendFs, scVol.VolDirBasePath)
	if err != nil {
		logger.Errorf(ctx, "volume:[%v] - unable to check if DirBasePath %v is present in filesystem %v. Error : %v", scVol.VolName, scVol.VolDirBasePath, scVol.VolBackendFs, err)
	...
	if !dirExists {
		logger.Errorf(ctx, "volume:[%v] - directory base path %v not present in filesystem %v", scVol.VolName, scVol.VolDirBasePath, scVol.VolBackendFs)
	...
	logger.Debugf(ctx, "volume: [%v] - creating directory %v", scVol.VolName, dirPath)
	err = cs.createDirectory(scVol, scVol.VolName, dirPath)
	if err != nil {
		logger.Errorf(ctx, "volume:[%v] - failed to create directory %v. Error : %v", scVol.VolName, dirPath, err)
...
func (cs *ScaleControllerServer) generateVolID(ctx context.Context, scVol *scaleVolume, uid string, isNewVolumeType bool, targetPath string) (string, error) {
	logger.Infof(ctx, "volume: [%v] - ControllerServer:generateVolId", scVol.VolName)
	var volID string
	...
	if isNewVolumeType {
		primaryConn, isprimaryConnPresent := cs.Driver.connmap["primary"]
		if !isprimaryConnPresent {
			logger.Errorf(ctx, "unable to get connector for primary cluster")
	...
	logger.Debugf(ctx, "volume: [%v] - ControllerServer:generateVolId: targetPath: [%v]", scVol.VolName, path)
...
func (cs *ScaleControllerServer) getTargetPath(fsetLinkPath, fsMountPoint, volumeName string, createDataDir bool, isNewVolumeType bool) (string, error) {
	if fsetLinkPath == "" || fsMountPoint == "" {
		logger.Errorf(eCtx, "volume:[%v] - missing details to generate target path fileset junctionpath: [%v], filesystem mount point: [%v]", volumeName, fsetLinkPath, fsMountPoint)
	...
	logger.Debugf(eCtx, "volume: [%v] - ControllerServer:getTargetPath", volumeName)
...
func (cs *ScaleControllerServer) createDirectory(scVol *scaleVolume, volName string, targetPath string) error {
	logger.Infof(eCtx, "volume: [%v] - ControllerServer:createDirectory", volName)
	dirExists, err := scVol.Connector.CheckIfFileDirPresent(scVol.VolBackendFs, targetPath)
	if err != nil {
		logger.Errorf(eCtx, "volume:[%v] - unable to check if directory path [%v] exists in filesystem [%v]. Error : %v", volName, targetPath, scVol.VolBackendFs, err)
	...
	if !dirExists {
		if scVol.VolPermissions != "" {
			...
			if err != nil {
				...
				logger.Errorf(eCtx, "volume:[%v] - unable to create directory [%v] in filesystem [%v]. Error : %v", volName, targetPath, scVol.VolBackendFs, err)
...
func (cs *ScaleControllerServer) createSoftlink(ctx context.Context, scVol *scaleVolume, target string) error {
	logger.Debugf(ctx, "volume: [%v] - ControllerServer:createSoftlink", scVol.VolName)
	...
	if err != nil {
		logger.Errorf(ctx, "volume:[%v] - unable to check if symlink path [%v] exists in filesystem [%v]. Error: %v", scVol.VolName, volSlnkPath, scVol.PrimaryFS, err)
		return fmt.Errorf("unable to check if symlink path [%v] exists in filesystem [%v]. Error: %v", volSlnkPath, scVol.PrimaryFS, err)
	}

	if !symLinkExists {
		logger.Infof(ctx, "symlink info filesystem [%v] TargetFS [%v]  target Path [%v] linkPath [%v]", scVol.PrimaryFS, scVol.LocalFS, target, volSlnkPath)
		err = scVol.PrimaryConnector.CreateSymLink(scVol.PrimaryFS, scVol.LocalFS, target, volSlnkPath)
		if err != nil {
			logger.Errorf(ctx, "volume:[%v] - failed to create symlink [%v] in filesystem [%v], for target [%v] in filesystem [%v]. Error [%v]", scVol.VolName, volSlnkPath, scVol.PrimaryFS, target, scVol.LocalFS, err)
...
func (cs *ScaleControllerServer) setQuota(ctx context.Context, scVol *scaleVolume, volName string) error {
	logger.Debugf(ctx, "volume: [%v] - ControllerServer:setQuota", volName)
...
func (cs *ScaleControllerServer) createFilesetBasedVol(ctx context.Context, scVol *scaleVolume, isNewVolumeType bool) (string, error) { //nolint:gocyclo,funlen
	logger.Infof(ctx, "volume: [%v] - ControllerServer:createFilesetBasedVol", scVol.VolName)
	opt := make(map[string]interface{})

	// fileset can not be created if filesystem is remote.
	logger.Infof(ctx, "check if volumes filesystem [%v] is remote or local for cluster [%v]", scVol.VolBackendFs, scVol.ClusterId)
	...
	if err != nil {
		if strings.Contains(err.Error(), "Invalid value in filesystemName") {
			logger.Errorf(ctx, "volume:[%v] - filesystem %s in not known to cluster %v. Error: %v", scVol.VolName, scVol.VolBackendFs, scVol.ClusterId, err)
		...
		logger.Errorf(ctx, "volume:[%v] - unable to check type of filesystem [%v]. Error: %v", scVol.VolName, scVol.VolBackendFs, err)
	...
	if fsDetails.Type == filesystemTypeRemote {
		logger.Errorf(ctx, "volume:[%v] - filesystem [%v] is not local to cluster [%v]", scVol.VolName, scVol.VolBackendFs, scVol.ClusterId)
	...
	if cs.Driver.primary.PrimaryCid != scVol.ClusterId {
		if fsDetails.Mount.Status != filesystemMounted {
			logger.Errorf(ctx, "volume:[%v] -  filesystem [%v] is [%v] on remote GUI of cluster [%v]", scVol.VolName, scVol.VolBackendFs, fsDetails.Mount.Status, scVol.ClusterId)
		...
		logger.Debugf(ctx, "volume:[%v] - mount point of volume filesystem [%v] on owning cluster is %v", scVol.VolName, scVol.VolBackendFs, fsDetails.Mount.MountPoint)
	}

	// check if quota is enabled on volume filesystem
	logger.Infof(ctx, "check if quota is enabled on filesystem [%v] ", scVol.VolBackendFs)
	if scVol.VolSize != 0 {
		err = scVol.Connector.CheckIfFSQuotaEnabled(scVol.VolBackendFs)
		if err != nil {
			logger.Errorf(ctx, "volume:[%v] - quota not enabled for filesystem %v of cluster %v. Error: %v", scVol.VolName, scVol.VolBackendFs, scVol.ClusterId, err)
	...
	if isNewVolumeType {
		...
		logger.Infof(ctx, "creating independent fileset for new storageClass with fileset name: [%v]", indepFilesetName)
		...
		if err != nil {
			logger.Errorf(ctx, "volume:[%v] - failed to create independent fileset [%v] in filesystem [%v]. Error: %v", indepFilesetName, indepFilesetName, scVol.VolBackendFs, err)
			return "", err
		}
		logger.Infof(ctx, "finished creation of independent fileset for new storageClass with fileset name: [%v]", indepFilesetName)

		// Now create dependent fileset
		logger.Infof(ctx, "creating dependent fileset for new storageClass with fileset name: [%v]", scVol.VolName)
		...
		// Create fileset
		logger.Infof(ctx, "creating fileset for classic storageClass with fileset name: [%v]", scVol.VolName)
		createDataDir := true
		filesetPath, err := cs.createFilesetVol(ctx, scVol, scVol.VolName, fsDetails, opt, createDataDir, false, isNewVolumeType)
		if err != nil {
			logger.Errorf(ctx, "volume:[%v] - failed to create fileset [%v] in filesystem [%v]. Error: %v", scVol.VolName, scVol.VolName, scVol.VolBackendFs, err)
			return "", err
		}
		logger.Infof(ctx, "finished creation of fileset for classic storageClass with fileset name: [%v]", scVol.VolName)
		return filesetPath, nil
	}
...
func (cs *ScaleControllerServer) createFilesetVol(ctx context.Context, scVol *scaleVolume, volName string, fsDetails connectors.FileSystem_v2, opt map[string]interface{}, createDataDir bool, isCGIndependentFset bool, isNewVolumeType bool) (string, error) { //nolint:gocyclo,funlen
	...
	if err != nil {
		if strings.Contains(err.Error(), "Invalid value in 'filesetName'") {
			...
			if fseterr != nil {
				...
				logger.Errorf(ctx, "volume:[%v] - unable to create fileset [%v] in filesystem [%v]. Error: %v", volName, volName, scVol.VolBackendFs, fseterr)
			...
			if err != nil {
				...
				logger.Errorf(ctx, "volume:[%v] - unable to list newly created fileset [%v] in filesystem [%v]. Error: %v", volName, volName, scVol.VolBackendFs, err)
			...
			logger.Errorf(ctx, "volume:[%v] - unable to list fileset [%v] in filesystem [%v]. Error: %v", volName, volName, scVol.VolBackendFs, err)
		...
		if filesetInfo.Config.Comment != connectors.FilesetComment {
			logger.Errorf(ctx, "volume:[%v] - the fileset is not created by IBM Spectrum Scale CSI driver. Cannot use it.", volName)
		...
		if opt[connectors.UserSpecifiedFilesetType] != listFilesetType {
			logger.Errorf(ctx, "volume:[%v] - the fileset type is not as expected, got type: [%s], expected type: [%s]", volName, listFilesetType, opt[connectors.UserSpecifiedFilesetType])
	...
	if (filesetInfo.Config.Path == "") || (filesetInfo.Config.Path == filesetUnlinkedPath) {
		...
		if scVol.ParentFileset != "" {
			...
			if err != nil {
				logger.Errorf(ctx, "volume:[%v] - unable to get details of parent fileset [%v] in filesystem [%v]. Error: %v", volName, scVol.ParentFileset, scVol.VolBackendFs, err)
			...
			if (parentfilesetInfo.Config.Path == "") || (parentfilesetInfo.Config.Path == filesetUnlinkedPath) {
				logger.Errorf(ctx, "volume:[%v] - parent fileset [%v] is not linked", volName, scVol.ParentFileset)
		...
		if err != nil {
			logger.Errorf(ctx, "volume:[%v] - linking fileset [%v] in filesystem [%v] at path [%v] failed. Error: %v", volName, volName, scVol.VolBackendFs, junctionPath, err)
		...
		filesetInfo, err = scVol.Connector.ListFileset(scVol.VolBackendFs, volName)
		if err != nil {
			logger.Errorf(ctx, "volume:[%v] - unable to list fileset [%v] in filesystem [%v] after linking. Error: %v", volName, volName, scVol.VolBackendFs, err)
...
func (cs *ScaleControllerServer) getConnFromClusterID(ctx context.Context, cid string) (connectors.SpectrumScaleConnector, error) {
	...
	logger.Errorf(ctx, "unable to get connector for cluster ID %v", cid)
...
func (cs *ScaleControllerServer) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) { //nolint:gocyclo,funlen
	...
	if volSrc != nil {
		...
		if srcVolume != nil {
			...
			if err != nil {
				logger.Errorf(ctx, "volume:[%v] - Invalid Volume ID %s [%v]", volName, srcVolumeID, err)
				return nil, err
			}
			isVolSource = true
		} else {
			...
			if srcSnap != nil {
				...
				if err != nil {
					logger.Errorf(ctx, "volume:[%v] - Invalid snapshot ID %s [%v]", volName, snapId, err)
					return nil, err
				}
				isSnapSource = true
			}
	...
	logger.Infof(ctx, "volume:[%v] - check if primary fileset [%v] is linked", scaleVol.VolName, primaryFileset)
	isPrimaryFilesetLinked, err := scaleVol.PrimaryConnector.IsFilesetLinked(ctx, scaleVol.PrimaryFS, primaryFileset)
	if err != nil {
		logger.Errorf(ctx, "volume:[%v] - unable to get details of Primary Fileset [%v]. Error : [%v]", scaleVol.VolName, primaryFileset, err)
	...
	if !isPrimaryFilesetLinked {
		logger.Errorf(ctx, "volume:[%v] - primary fileset [%v] is not linked", scaleVol.VolName, primaryFileset)
	...
	if scaleVol.PrimaryFS != scaleVol.VolBackendFs {
		...
		logger.Debugf(ctx, "volume:[%v] - check if primary filesystem [%v] is mounted on GUI node of Primary cluster", scaleVol.VolName, scaleVol.PrimaryFS)
		isPfsMounted, err := scaleVol.PrimaryConnector.IsFilesystemMountedOnGUINode(scaleVol.PrimaryFS)
		if err != nil {
			logger.Errorf(ctx, "volume:[%v] - unable to get filesystem mount details for %s on Primary cluster. Error: %v", scaleVol.VolName, scaleVol.PrimaryFS, err)
		...
		if !isPfsMounted {
			logger.Errorf(ctx, "volume:[%v] - primary filesystem %s is not mounted on GUI node of Primary cluster", scaleVol.VolName, scaleVol.PrimaryFS)
	...
	logger.DebugPlus(ctx, "volume:[%v] - check if volume filesystem [%v] is mounted on GUI node of Primary cluster", scaleVol.VolName, scaleVol.VolBackendFs)
	volFsInfo, err := scaleVol.PrimaryConnector.GetFilesystemDetails(ctx, scaleVol.VolBackendFs)
	if err != nil {
		if strings.Contains(err.Error(), "Invalid value in filesystemName") {
			logger.Errorf(ctx, "volume:[%v] - filesystem %s in not known to primary cluster. Error: %v", scaleVol.VolName, scaleVol.VolBackendFs, err)
		...
		logger.Errorf(ctx, "volume:[%v] - unable to get details for filesystem [%v] in Primary cluster. Error: %v", scaleVol.VolName, scaleVol.VolBackendFs, err)
	...
	if volFsInfo.Mount.Status != filesystemMounted {
		logger.Errorf(ctx, "volume:[%v] - volume filesystem %s is not mounted on GUI node of Primary cluster", scaleVol.VolName, scaleVol.VolBackendFs)
	...
	logger.DebugPlus(ctx, "volume:[%v] - mount point of volume filesystem [%v] is on Primary cluster is %v", scaleVol.VolName, scaleVol.VolBackendFs, volFsInfo.Mount.MountPoint)
	...
	if scaleVol.ClusterId == "" && volFsInfo.Type == filesystemTypeRemote {
		logger.Infof(ctx, "filesystem %s is remotely mounted, getting cluster ID information of the owning cluster.", volFsInfo.Name)
		...
		logger.DebugPlus(ctx, "cluster ID for remote cluster %s is %s", clusterName, remoteClusterID)
	}

	if scaleVol.IsFilesetBased {
		if scaleVol.ClusterId == "" {
			if volFsInfo.Type == filesystemTypeRemote { // if fileset based and remotely mounted.
				logger.Infof(ctx, "volume filesystem %s is remotely mounted on Primary cluster, using owning cluster ID %s.", scaleVol.LocalFS, remoteClusterID)
				scaleVol.ClusterId = remoteClusterID
			} else {
				logger.Infof(ctx, "volume filesystem %s is locally mounted on Primary cluster, using primary cluster ID %s.", scaleVol.LocalFS, PCid)
	...
	if isVolSource {
		...
		if err != nil {
			logger.Errorf(ctx, "volume:[%v] - Error in source volume validation [%v]", volName, err)
			return nil, err
		}
	...
	if isSnapSource {
		...
		if err != nil {
			logger.Errorf(ctx, "volume:[%v] - Error in source snapshot validation [%v]", volName, err)
			return nil, err
		}
	}

	logger.Infof(ctx, "volume:[%v] -  spectrum scale volume create params : %v\n", scaleVol.VolName, scaleVol)

	if scaleVol.IsFilesetBased && scaleVol.Compression != "" {
		logger.Infof(ctx, "createvolume: compression is enabled: changing volume name")
	...
	if scaleVol.IsFilesetBased && scaleVol.Tier != "" {
		...
		if err != nil {
			logger.Errorf(ctx, "volume:[%v] - setting policy failed [%v]", volName, err)
			return nil, err
		}
		...
		if !scaleVol.Connector.CheckIfDefaultPolicyPartitionExists(ctx, defaultPartitionName, scaleVol.VolBackendFs) {
			logger.Infof(ctx, "createvolume: setting default policy partition rule")
			...
			if err != nil {
				logger.Errorf(ctx, "volume:[%v] - setting default policy failed [%v]", volName, err)
				return nil, err
			}
		}
	}
	...
	if volReqInProcess {
		logger.Errorf(ctx, "volume:[%v] - volume creation already in process ", scaleVol.VolName)
	...
	if isVolSource {
		...
		if found {
			...
			logger.DebugPlus(ctx, "volume: [%v] found in volcopyjobstatusmap with volID: [%v], jobStatus: [%v]", scaleVol.VolName, volID, jobStatus)
			switch jobStatus {
			case VOLCOPY_JOB_RUNNING:
				logger.Errorf(ctx, "volume:[%v] -  volume cloning request in progress.", scaleVol.VolName)
			...
			case VOLCOPY_JOB_FAILED:
				logger.Errorf(ctx, "volume:[%v] -  volume cloning job had failed", scaleVol.VolName)
			...
			case VOLCOPY_JOB_COMPLETED:
				logger.Infof(ctx, "volume:[%v] -  volume cloning request has already completed successfully.", scaleVol.VolName)
			...
			case JOB_STATUS_UNKNOWN:
				...
				logger.Infof(ctx, "volume:[%v] -  the status of volume cloning job is unknown.", scaleVol.VolName)
				cs.Driver.volcopyjobstatusmap.Delete(scaleVol.VolName)
			}
		} else {
			logger.Infof(ctx, "volume: [%v] not found in volcopyjobstatusmap", scaleVol.VolName)
		}
	}
	...
	if isSnapSource {
		...
		if found {
			...
			logger.DebugPlus(ctx, "volume: [%v] found in snapjobstatusmap with volID: [%v], jobStatus: [%v]", scaleVol.VolName, volID, jobStatus)
			switch jobStatus {
			case SNAP_JOB_RUNNING:
				logger.Errorf(ctx, "volume:[%v] -  snapshot copy request in progress for snapshot: %s.", scaleVol.VolName, snapIdMembers.SnapName)
			...
			case SNAP_JOB_FAILED:
				logger.Errorf(ctx, "volume:[%v] -  snapshot copy job had failed for snapshot %s", scaleVol.VolName, snapIdMembers.SnapName)
			...
			case SNAP_JOB_COMPLETED:
				logger.DebugPlus(ctx, "volume:[%v] -  snapshot copy request has already completed successfully for snapshot: %s", scaleVol.VolName, snapIdMembers.SnapName)
				return &csi.CreateVolumeResponse{
					Volume: &csi.Volume{
						VolumeId:      volID,
						CapacityBytes: int64(scaleVol.VolSize),
			...
			case JOB_STATUS_UNKNOWN:
				//Remove the entry from map, so that it can be retried
				logger.DebugPlus(ctx, "volume:[%v] -  the status of snapshot copy job for snapshot [%s] is unknown", scaleVol.VolName, snapIdMembers.SnapName)
				cs.Driver.snapjobstatusmap.Delete(scaleVol.VolName)
			}
		} else {
			logger.DebugPlus(ctx, "volume: [%v] not found in snapjobstatusmap", scaleVol.VolName)
		}
	}
	...
	if isVolSource {
		...
		if err != nil {
			logger.Errorf(ctx, "CreateVolume [%s]: [%v]", volName, err)
			return nil, err
		}
	}
	...
	if isSnapSource {
		...
		if err != nil {
			logger.Errorf(ctx, "createVolume failed while copying snapshot content [%s]: [%v]", volName, err)
			return nil, err
		}
	}
...
func (cs *ScaleControllerServer) copySnapContent(ctx context.Context, scVol *scaleVolume, snapId scaleSnapId, fsDetails connectors.FileSystem_v2, targetPath string, volID string) error {
	logger.Infof(ctx, "copySnapContent snapId: [%v], scaleVolume: [%v]", snapId, scVol)
	...
	if err != nil {
		logger.Errorf(ctx, "failed to create volume from snapshot %s: [%v]", snapId.SnapName, err)
	...
	if err != nil || isResponseStatusUnknown {
		logger.Errorf(ctx, "unable to copy snapshot %s: %v.", snapId.SnapName, err)
	...
	logger.Infof(ctx, "copy snapshot completed for snapId: [%v], scaleVolume: [%v]", snapId, scVol)
...
func (cs *ScaleControllerServer) copyVolumeContent(ctx context.Context, newvolume *scaleVolume, sourcevolume scaleVolId, fsDetails connectors.FileSystem_v2, targetPath string, volID string) error {
	logger.Infof(ctx, "copyVolContent volume ID: [%v], scaleVolume: [%v], volume name: [%v]", sourcevolume, newvolume, newvolume.VolName)
	...
	if newvolume.IsFilesetBased {
		...
		if jobErr != nil {
			logger.Errorf(ctx, "failed to clone volume from volume. Error: [%v]", jobErr)
	...
	if err != nil || isResponseStatusUnknown {
		...
		logger.Errorf(ctx, "logging volume cloning error for VolName: [%v] Error: [%v] JobDetails: [%v]", newvolume.VolName, err, jobDetails)
		cs.Driver.volcopyjobstatusmap.Store(newvolume.VolName, jobDetails)
		return err
	}

	logger.Infof(ctx, "volume copy completed for volumeID: [%v], scaleVolume: [%v]", sourcevolume, newvolume)
...
func (cs *ScaleControllerServer) checkMinFsVersion(fsVersion string, version string) bool {
	...
	logger.Infof(eCtx, "fs version (%s) vs min required version (%s)", assembledFsVer, version)
...
func (cs *ScaleControllerServer) validateSnapId(ctx context.Context, sourcesnapshot *scaleSnapId, newvolume *scaleVolume, pCid string) error {
	logger.Infof(ctx, "validateSnapId [%v]", sourcesnapshot)
...
func (cs *ScaleControllerServer) validateCloneRequest(ctx context.Context, sourcevolume *scaleVolId, newvolume *scaleVolume, pCid string, volFsInfo connectors.FileSystem_v2) error {
	logger.Infof(ctx, "validateVolId [%v]", sourcevolume)
...
func (cs *ScaleControllerServer) DeleteFilesetVol(ctx context.Context, FilesystemName string, FilesetName string, volumeIdMembers scaleVolId, conn connectors.SpectrumScaleConnector) (bool, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "EFSSG0072C") ||
			...
			logger.Debugf(ctx, "fileset seems already deleted - %v", err)
			return true, nil
		}
	...
	logger.Infof(ctx, "there is no snapshot present in the fileset [%v], continue DeleteFilesetVol", FilesetName)
...
func (cs *ScaleControllerServer) DeleteCGFileset(ctx context.Context, FilesystemName string, volumeIdMembers scaleVolId, conn connectors.SpectrumScaleConnector) error {
	logger.Infof(ctx, "Trying to delete independent fileset for consistency group [%v]", volumeIdMembers.ConsistencyGroup)
	...
	if filesetDetails.Config.Comment == connectors.FilesetComment {
		...
		if len(filesets) > 1 {
			logger.Debugf(ctx, "Found atleast one dependent fileset for consistency group: [%v]", volumeIdMembers.ConsistencyGroup)
			return nil
		}
		...
		logger.Infof(ctx, "Deleted independent fileset for consistency group [%v]", volumeIdMembers.ConsistencyGroup)
	} else {
		logger.Infof(ctx, "Independent fileset for consistency group [%v] not created by IBM Spectrum Scale CSI Driver. Cannot delete it.", volumeIdMembers.ConsistencyGroup)
	}
...
func (cs *ScaleControllerServer) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	logger.Infof(ctx, "DeleteVolume [%v]", req)

	if err := cs.Driver.ValidateControllerServiceRequest(ctx, csi.ControllerServiceCapability_RPC_CREATE_DELETE_VOLUME); err != nil {
		logger.Errorf(ctx, "Invalid delete volume req: %v", req)
	...
	logger.Debugf(ctx, "Volume Id Members [%v]", volumeIdMembers)
	...
	if volumeIdMembers.IsFilesetBased {
		...
		if FilesetName != "" {
			...
			if pvName == FilesetName {
				...
				logger.Infof(ctx, "pv name from path [%v] does not match with filesetName [%v]. Skipping delete of fileset", pvName, FilesetName)
			}
		}
	} else {
...
func (cs *ScaleControllerServer) ControllerGetCapabilities(ctx context.Context, req *csi.ControllerGetCapabilitiesRequest) (*csi.ControllerGetCapabilitiesResponse, error) {
	logger.Infof(ctx, "ControllerGetCapabilities called with req: %#v", req)
...
func (cs *ScaleControllerServer) ValidateVolumeCapabilities(ctx context.Context, req *csi.ValidateVolumeCapabilitiesRequest) (*csi.ValidateVolumeCapabilitiesResponse, error) {
	volumeID := req.GetVolumeId()
	logger.Debugf(ctx, "ValidateVolumeCapabilities called with req: %#v", req)
	if volumeID == "" {
...
func (cs *ScaleControllerServer) ControllerUnpublishVolume(ctx context.Context, req *csi.ControllerUnpublishVolumeRequest) (*csi.ControllerUnpublishVolumeResponse, error) {
	logger.Infof(ctx, "controllerserver ControllerUnpublishVolume")
	logger.Debugf(ctx, "ControllerUnpublishVolume : req %#v", req)
	...
	if err := cs.Driver.ValidateControllerServiceRequest(ctx, csi.ControllerServiceCapability_RPC_PUBLISH_UNPUBLISH_VOLUME); err != nil {
		logger.Errorf(ctx, "invalid Unpublish volume request: %v", req)
...
func (cs *ScaleControllerServer) ControllerPublishVolume(ctx context.Context, req *csi.ControllerPublishVolumeRequest) (*csi.ControllerPublishVolumeResponse, error) { //nolint:gocyclo,funlen
	logger.Infof(ctx, "Controllerserver ControllerPublishVolume")
	logger.Debugf(ctx, "ControllerPublishVolume : req %#v", req)

	if err := cs.Driver.ValidateControllerServiceRequest(ctx, csi.ControllerServiceCapability_RPC_PUBLISH_UNPUBLISH_VOLUME); err != nil {
		logger.Errorf(ctx, "Invalid Publish volume request: %v", req)
	...
	logger.Infof(ctx, "ControllerPublishVolume : SKIP_MOUNT_UNMOUNT is set to %s", skipMountUnmount)

	//Get filesystem name from UUID
	fsName, err := cs.Driver.connmap["primary"].GetFilesystemName(ctx, filesystemID)
	if err != nil {
		logger.Errorf(ctx, "ControllerPublishVolume : Error in getting filesystem Name for filesystem ID of %s.", filesystemID)
	...
	if err != nil {
		logger.Errorf(ctx, "ControllerPublishVolume : Error in getting filesystem mount details for %s", primaryfsName)
	...
	logger.Infof(ctx, "ControllerUnpublishVolume : scalenodeID:%s --known as-- k8snodeName: %s", scalenodeID, nodeID)

	shortnameNodeMapping := utils.GetEnv(SHORTNAME_NODE_MAPPING, no)
	if shortnameNodeMapping == yes {
		logger.Debugf(ctx, "ControllerPublishVolume : SHORTNAME_NODE_MAPPING is set to %s", shortnameNodeMapping)
	}
	...
	logger.Infof(ctx, "ControllerPublishVolume : Primary FS is mounted on %v", pfsMount.NodesMounted)
	logger.Debugf(ctx, "ControllerPublishVolume : Primary Fileystem is %s and Volume is from Filesystem %s", primaryfsName, fsName)
	...
	if volumeIDMembers.StorageClassType == STORAGECLASS_ADVANCED || primaryfsName != fsName {
		...
		if err != nil {
			logger.Errorf(ctx, "ControllerPublishVolume : Error in getting filesystem mount details for %s", fsName)
		...
		if volumeIDMembers.StorageClassType == STORAGECLASS_ADVANCED &&
			...
			logger.Errorf(ctx, "ControllerPublishVolume : Volume path %s is not part of the filesystem %s", volumePath, fsName)
			...
			logger.Errorf(ctx, "ControllerPublishVolume : Volume path %s is not part of the filesystem %s or %s", volumePath, primaryfsName, fsName)
		...
		logger.Infof(ctx, "ControllerPublishVolume : Volume Source FS is mounted on %v", fsMount.NodesMounted)
	} else {
		if !strings.HasPrefix(volumePath, pfsMount.MountPoint) {
			logger.Errorf(ctx, "ControllerPublishVolume : Volume path %s is not part of the filesystem %s", volumePath, primaryfsName)
	...
	logger.Infof(ctx, "ControllerPublishVolume : Mount Status Primaryfs [ %t ], Sourcefs [ %t ]", ispFsMounted, isFsMounted)

	if isFsMounted && ispFsMounted {
		logger.Debugf(ctx, "ControllerPublishVolume : %s and %s are mounted on %s so returning success", fsName, primaryfsName, scalenodeID)
		return &csi.ControllerPublishVolumeResponse{}, nil
	}

	if skipMountUnmount == "yes" && (!isFsMounted || !ispFsMounted) {
		logger.Errorf(ctx, "ControllerPublishVolume : SKIP_MOUNT_UNMOUNT == yes and either %s or %s is not mounted on node %s", primaryfsName, fsName, scalenodeID)
	...
	if !(ispFsMounted) && skipMountUnmount == no {
		logger.Debugf(ctx, "ControllerPublishVolume : mounting Filesystem %s on %s", primaryfsName, scalenodeID)
		err = cs.Driver.connmap["primary"].MountFilesystem(primaryfsName, scalenodeID)
		if err != nil {
			logger.Errorf(ctx, "ControllerPublishVolume : Error in mounting filesystem %s on node %s", primaryfsName, scalenodeID)
	...
	if !(isFsMounted) && skipMountUnmount == no && primaryfsName != fsName {
		logger.Debugf(ctx, "ControllerPublishVolume : mounting %s on %s", fsName, scalenodeID)
		err = cs.Driver.connmap["primary"].MountFilesystem(fsName, scalenodeID)
		if err != nil {
			logger.Errorf(ctx, "ControllerPublishVolume : Error in mounting filesystem %s on node %s", fsName, scalenodeID)
...
func (cs *ScaleControllerServer) CheckNewSnapRequired(ctx context.Context, conn connectors.SpectrumScaleConnector, filesystemName string, filesetName string, snapWindow int) (string, error) {
	...
	if err != nil {
		logger.Errorf(ctx, "CheckNewSnapRequired - getting latest snapshot list failed for fileset: [%s:%s]. Error: [%v]", filesystemName, filesetName, err)
		return "", err
	}
	...
	if err != nil {
		logger.Errorf(ctx, "Error getting create timestamp for snapshot %s:%s:%s", filesystemName, filesetName, latestSnapList[0].SnapshotName)
		return "", err
	}
	...
	logger.Infof(ctx, "Fileset [%s:%s], last snapshot time: [%v], current time: [%v], passed time: %v seconds, snapWindow: %v minutes", filesystemName, filesetName, lastSnapTime, time.Now(), int64(passedTime), snapWindow)

	snapWindowSeconds := snapWindow * 60

	if passedTime < float64(snapWindowSeconds) {
		// we don't need to take new snapshot
		logger.Infof(ctx, "CheckNewSnapRequired - for fileset [%s:%s], using existing snapshot [%s]", filesystemName, filesetName, latestSnapList[0].SnapshotName)
		return latestSnapList[0].SnapshotName, nil
	}

	logger.Infof(ctx, "CheckNewSnapRequired - for fileset [%s:%s] we need to create new snapshot", filesystemName, filesetName)
	return "", nil
}
...
func (cs *ScaleControllerServer) MakeSnapMetadataDir(ctx context.Context, conn connectors.SpectrumScaleConnector, filesystemName string, filesetName string, indepFileset string, cgSnapName string, metaSnapName string) error {
	...
	logger.Infof(ctx, "MakeSnapMetadataDir - creating directory [%s] for fileset: [%s:%s]", path, filesystemName, filesetName)
	err := conn.MakeDirectory(filesystemName, path, "0", "0")
	if err != nil {
		// Directory creation failed
		logger.Errorf(ctx, "Volume:[%v] - unable to create directory [%v] in filesystem [%v]. Error : %v", filesetName, path, filesystemName, err)
...
func (cs *ScaleControllerServer) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (*csi.CreateSnapshotResponse, error) { //nolint:gocyclo,funlen
	logger.Infof(ctx, "CreateSnapshot - create snapshot req: %v", req)

	if err := cs.Driver.ValidateControllerServiceRequest(ctx, csi.ControllerServiceCapability_RPC_CREATE_DELETE_SNAPSHOT); err != nil {
		logger.Errorf(ctx, "CreateSnapshot - invalid create snapshot req: %v", req)
	...
	primaryConn, isprimaryConnPresent := cs.Driver.connmap["primary"]
	if !isprimaryConnPresent {
		logger.Errorf(ctx, "CreateSnapshot - unable to get connector for primary cluster")
	...
	relPath := ""
	if volumeIDMembers.StorageClassType == STORAGECLASS_ADVANCED {
		logger.Debugf(ctx, "CreateSnapshot - creating snapshot for advanced storageClass")
		relPath = strings.Replace(volumeIDMembers.Path, mountInfo.MountPoint, "", 1)
	} else {
		logger.Debugf(ctx, "CreateSnapshot - creating snapshot for classic storageClass")
		relPath = strings.Replace(volumeIDMembers.Path, cs.Driver.primary.PrimaryFSMount, "", 1)
	}
	relPath = strings.Trim(relPath, "!/")
	...
	if volumeIDMembers.StorageClassType == STORAGECLASS_ADVANCED {
		...
		if !snapWindowSpecified {
			...
			logger.Infof(ctx, "SnapWindow not specified. Using default snapWindow: [%s] for for fileset[%s:%s]", snapWindow, filesetResp.FilesetName, filesystemName)
		}
	...
	if err != nil {
		logger.Errorf(ctx, "CreateSnapshot [%s] - Unable to get the snapshot details. Error [%v]", snapName, err)
	...
	if !snapExist {
		...
		if volumeIDMembers.StorageClassType == STORAGECLASS_ADVANCED {
			...
			if err != nil {
				logger.Errorf(ctx, "CreateSnapshot [%s] - unable to check if snapshot is required for new storageClass for fileset [%s:%s]. Error: [%v]", snapName, filesystemName, filesetName, err)
				return nil, err
			}
			if cgSnapName != "" {
				...
				logger.Infof(ctx, "CreateSnapshot - creating new snapshot for consistency group for fileset: [%s:%s]", filesystemName, filesetName)
			}
		}
		...
		if createNewSnap {
			snapshotList, err := conn.ListFilesetSnapshots(ctx, filesystemName, filesetName)
			if err != nil {
				logger.Errorf(ctx, "CreateSnapshot [%s] - unable to list snapshots for fileset [%s:%s]. Error: [%v]", snapName, filesystemName, filesetName, err)
			...
			if len(snapshotList) >= 256 {
				logger.Errorf(ctx, "CreateSnapshot [%s] - max limit of snapshots reached for fileset [%s:%s]. No more snapshots can be created for this fileset.", snapName, filesystemName, filesetName)
			...
			if snaperr != nil {
				logger.Errorf(ctx, "Snapshot [%s] - Unable to create snapshot. Error [%v]", snapName, snaperr)
	...
	if err != nil {
		logger.Errorf(ctx, "Error getting create timestamp for snapshot %s:%s:%s", filesystemName, filesetName, snapName)
		return nil, err
	}

	restoreSize, err := cs.getSnapRestoreSize(conn, filesystemName, filesetResp.FilesetName)
	if err != nil {
		logger.Errorf(ctx, "Error getting the snapshot restore size for snapshot %s:%s:%s", filesystemName, filesetResp.FilesetName, snapName)
		return nil, err
	}
	...
	if volumeIDMembers.StorageClassType == STORAGECLASS_ADVANCED {
		...
		if err != nil {
			logger.Errorf(ctx, "Error in creating directory for storing metadata information for advanced storageClass. Error: [%v]", err)
			return nil, err
		}
	}
...
func (cs *ScaleControllerServer) getSnapshotCreateTimestamp(conn connectors.SpectrumScaleConnector, fs string, fset string, snap string) (timestamp.Timestamp, error) {
	...
	if err != nil {
		logger.Errorf(eCtx, "snapshot [%s] - Unable to get snapshot create timestamp", snap)
		return timestamp, err
	}

	timezoneOffset, err := conn.GetTimeZoneOffset()
	if err != nil {
		logger.Errorf(eCtx, "snapshot [%s] - Unable to get cluster timezone", snap)
		return timestamp, err
	}
	...
	if err != nil {
		logger.Errorf(eCtx, "snapshot - for fileset [%s:%s] error in parsing timestamp: [%v]. Error: [%v]", fs, fset, createTS, err)
		return timestamp, err
	}
	timestamp.Seconds = t.Unix()
	timestamp.Nanos = 0

	logger.Infof(eCtx, "getSnapshotCreateTimestamp: for fileset [%s:%s] snapshot creation timestamp: [%v]", fs, fset, createTSTZ)
...
func (cs *ScaleControllerServer) getSnapRestoreSize(conn connectors.SpectrumScaleConnector, filesystemName string, filesetName string) (int64, error) {
	...
	if quotaResp.BlockLimit < 0 {
		logger.Errorf(eCtx, "getSnapRestoreSize: Invalid block limit [%v] for fileset [%s:%s] found", quotaResp.BlockLimit, filesystemName, filesetName)
...
func (cs *ScaleControllerServer) DelSnapMetadataDir(ctx context.Context, conn connectors.SpectrumScaleConnector, filesystemName string, consistencyGroup string, filesetName string, cgSnapName string, metaSnapName string) (bool, error) {
	...
	logger.Infof(ctx, "DelSnapMetadataDir - number of links for directory in FS [%v] at path [%v] is [%v]", filesystemName, pathDir, nlink)
...
func (cs *ScaleControllerServer) DeleteSnapshot(ctx context.Context, req *csi.DeleteSnapshotRequest) (*csi.DeleteSnapshotResponse, error) {
	logger.Infof(ctx, "DeleteSnapshot - delete snapshot req: %v", req)

	if err := cs.Driver.ValidateControllerServiceRequest(ctx, csi.ControllerServiceCapability_RPC_CREATE_DELETE_SNAPSHOT); err != nil {
		logger.Errorf(ctx, "DeleteSnapshot - invalid delete snapshot req %v: %v", req, err)
	...
	snapIdMembers, err := cs.GetSnapIdMembers(snapID)
	if err != nil {
		logger.Errorf(ctx, "Invalid snapshot ID %s [%v]", snapID, err)
		return nil, err
	}
	...
	if filesetExist {
		snapExist := false
		if snapIdMembers.StorageClassType == STORAGECLASS_ADVANCED {
			logger.Debugf(ctx, "DeleteSnapshot - for advanced storageClass check if snapshot [%s] exist in fileset [%s] under filesystem [%s]", snapIdMembers.SnapName, snapIdMembers.ConsistencyGroup, filesystemName)
			...
			logger.Debugf(ctx, "DeleteSnapshot - for classic storageClass check if snapshot [%s] exist in fileset [%s] under filesystem [%s]", snapIdMembers.SnapName, snapIdMembers.FsetName, filesystemName)
		...
		if snapExist {
			...
			if snapIdMembers.StorageClassType == STORAGECLASS_ADVANCED {
				...
				if snaperr != nil {
					logger.Errorf(ctx, "DeleteSnapshot - error while deleting snapshot %v: Error: %v", snapIdMembers.SnapName, snaperr)
					return nil, snaperr
				}
				if delSnap {
					filesetName = snapIdMembers.ConsistencyGroup
					logger.Debugf(ctx, "DeleteSnapshot - for advanced storageClass we can delete snapshot [%s] from fileset [%s] under filesystem [%s]", snapIdMembers.SnapName, filesetName, filesystemName)
				} else {
			...
			if deleteSnapshot {
				logger.Infof(ctx, "DeleteSnapshot - deleting snapshot [%s] from fileset [%s] under filesystem [%s]", snapIdMembers.SnapName, filesetName, filesystemName)
				snaperr := conn.DeleteSnapshot(ctx, filesystemName, filesetName, snapIdMembers.SnapName)
				if snaperr != nil {
					logger.Errorf(ctx, "DeleteSnapshot - error deleting snapshot %v: %v", snapIdMembers.SnapName, snaperr)
					return nil, snaperr
				}
				logger.Infof(ctx, "DeleteSnapshot - successfully deleted snapshot [%s] from fileset [%s] under filesystem [%s]", snapIdMembers.SnapName, filesetName, filesystemName)
			}
		}
	}
...
func (cs *ScaleControllerServer) ControllerExpandVolume(ctx context.Context, req *csi.ControllerExpandVolumeRequest) (*csi.ControllerExpandVolumeResponse, error) {
	logger.Infof(ctx, "ControllerExpandVolume - Volume expand req: %v", req)

	if err := cs.Driver.ValidateControllerServiceRequest(ctx, csi.ControllerServiceCapability_RPC_EXPAND_VOLUME); err != nil {
		logger.Errorf(ctx, "ControllerExpandVolume - invalid expand volume req: %v", req)
	...
	if err != nil {
		logger.Errorf(ctx, "ControllerExpandVolume - Error in source Volume ID %v: %v", volID, err)
	...
	if err != nil {
		logger.Errorf(ctx, "ControllerExpandVolume - unable to get filesystem Name for Filesystem Uid [%v] and clusterId [%v]. Error [%v]", volumeIDMembers.FsUUID, volumeIDMembers.ClusterId, err)
	...
	if err != nil {
		logger.Errorf(ctx, "unable to check fileset [%v] existance in filesystem [%v]. Error [%v]", filesetName, filesystemName, err)
	...
	if !fsetExist {
		logger.Errorf(ctx, "Fileset [%v] does not exist in filesystem [%v]. Error [%v]", filesetName, filesystemName, err)
	...
	if err != nil {
		logger.Errorf(ctx, "unable to list quota for fileset [%v] in filesystem [%v]. Error [%v]", filesetName, filesystemName, err)
	...
	filesetQuotaBytes, err := ConvertToBytes(quota)
	if err != nil {
		logger.Errorf(ctx, "unable to convert quota for fileset [%v] in filesystem [%v]. Error [%v]", filesetName, filesystemName, err)
	...
	if filesetQuotaBytes < capacity {
		...
		if err != nil {
			logger.Errorf(ctx, "unable to update the quota. Error [%v]", err)
	...
	if fsetDetails.Config.ParentId == 0 {
		if capacity > 10*oneGB {
			if numberInSlice(fsetDetails.Config.MaxNumInodes, maxInodesCombination) {
				...
				if fseterr != nil {
					logger.Errorf(ctx, "Volume:[%v] - unable to update fileset [%v] in filesystem [%v]. Error: %v", filesetName, filesetName, filesystemName, fseterr)
...
func (cs *ScaleControllerServer) getRemoteClusterID(ctx context.Context, clusterName string) (string, error) {
	logger.Debugf(ctx, "Fetching cluster details from cache map for cluster %s", clusterName)
	clusterDetails, found := cs.Driver.clusterMap.Load(ClusterName{clusterName})
	if found {
		logger.Debugf(ctx, "Checking if cluster details found from cache map for cluster %s has expired.", clusterName)
		if expired := checkExpiry(clusterDetails); !expired { // cluster details are not expired.
			logger.Debugf(ctx, "Cluster details found from cache map for cluster %s are valid.", clusterName)
			return clusterDetails.(ClusterDetails).id, nil
		} else { // cluster details are expired
			logger.Debugf(ctx, "cluster details found from cache map for cluster %s are expired.", clusterName)
			...
			if cName == clusterName {
				logger.Debugf(ctx, "updating cluster details in cache map for cluster %s.", clusterName)
				...
				logger.Debugf(ctx, "ClusterMap updated, [%s : %s]", cID, cName)
				return cID, nil
			} else {
				found = false
			}
	...
	if !found {
		logger.Debugf(ctx, "Cluster details are either expired or not found in cache map for cluster %s. Updating the cache map.", clusterName)
		scaleconfig := settings.LoadScaleConfigSettings()

		for i := range scaleconfig.Clusters {

			cID := scaleconfig.Clusters[i].ID
			logger.Debugf(ctx, "Fetching cluster details from cache map for cluster %s", scaleconfig.Clusters[i].ID)
			clusterDetails, found := cs.Driver.clusterMap.Load(ClusterID{cID})
			if found {
				logger.Debugf(ctx, "Checking if cluster details found from cache map for cluster %s has expired.", scaleconfig.Clusters[i].ID)
				if expired := checkExpiry(clusterDetails); !expired {
					logger.Debugf(ctx, "Cluster details found from cache map for cluster %s are valid.", scaleconfig.Clusters[i].ID)
					cName := clusterDetails.(ClusterDetails).name
					if cName == clusterName {
						return cID, nil
					}
				} else {
					logger.Debugf(ctx, "Cluster details found from cache map for cluster %s are expired.", scaleconfig.Clusters[i].ID)
					logger.Debugf(ctx, "Updating cluster details in cache map for cluster %s.", scaleconfig.Clusters[i].ID)
				...
				logger.Debugf(ctx, "Cluster details not found in cache map for cluster %s.", scaleconfig.Clusters[i].ID)
				logger.Debugf(ctx, "adding cluster details in cache map for cluster %s.", scaleconfig.Clusters[i].ID)
				cName, updated := cs.updateClusterMap(ctx, cID)
				if !updated {
					continue
				}
...
func (cs *ScaleControllerServer) updateClusterMap(ctx context.Context, cID string) (string, bool) {
	logger.Debugf(ctx, "Creating new connector for the cluster %s", cID)
	clusterConnector, err := cs.getConnFromClusterID(ctx, cID)
	// clusterConnector, err := connectors.NewSpectrumRestV2(cluster)
	if err != nil {
		logger.Debugf(ctx, "unable to create new connector for the cluster %s", cID)
		return "", false
	}

	clusterSummary, err := clusterConnector.GetClusterSummary()
	if err != nil {
		logger.Debugf(ctx, "unable to get cluster summary for cluster %s", cID)
		return "", false
	}
