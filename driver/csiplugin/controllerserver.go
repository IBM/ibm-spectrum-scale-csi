...
func (cs *ScaleControllerServer) DeleteSnapshot(ctx context.Context, req *csi.DeleteSnapshotRequest) (*csi.DeleteSnapshotResponse, error) {
	...
	if filesetExist {
		...
		if snapExist {
			...
			if refVolumeExists{
				return nil, status.Error(codes.Internal, fmt.Sprintf("error deleting snapshot as there is shallow copy volume reference [%v]. Error: [%v]", snapIdMembers.SnapName, err))
			}
