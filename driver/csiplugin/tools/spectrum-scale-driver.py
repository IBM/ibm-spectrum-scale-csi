...
def configureDriver(conf_dict, infile, outfile):
        """Configures Storage Scale driver from config file.
...
if len(sys.argv) != 2 or os.path.isfile(sys.argv[1]) == False :
      ...
      print "    This command configures the Storage Scale Driver. Ensure that the environment variable 'CSI_SCALE_PATH' is set to the sample files base path. Configure the CSI driver parameters in spectrum-scale-driver.conf and run 'spectrum-scale-driver.py <path_to_spectrum-scale-driver.conf'"
      exit(1)
...
print "Storage Scale CSI driver configuration is complete. Please review the configuration and run 'deploy/classic/create.sh' to deploy the driver"
