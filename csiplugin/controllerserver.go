...
func (cs *ScaleControllerServer) CreateFilesetBasedVol(scVol *scaleVolume) (error) {
...
/* If we reach here we need to create a volume */
        ...
        if scaleVol.IsFilesetBased {
                err = cs.CreateFilesetBasedVol(scaleVol)
        } else {
        ...
        if scaleVol.IsFilesetBased {
                ...
                if err != nil {
                        cs.Cleanup(scaleVol)
                        return nil, status.Error(codes.Internal, fmt.Sprintf("Unable to list Fset [%v] in FS [%v]. Error [%v]",scaleVol.VolName, scaleVol.VolBackendFs, err))
                }

                linkpath := fsetResponse.Config.Path
                fsMount, err := scaleVol.Connector.GetFilesystemMountDetails(scaleVol.VolBackendFs)
                if err != nil {
                        cs.Cleanup(scaleVol)
                        return nil, status.Error(codes.Internal, fmt.Sprintf("Unable to fetch mount details for FS %v", scaleVol.VolBackendFs))
                }

                if fsMount.NodesMounted == nil || len(fsMount.NodesMounted) == 0 {
                    return nil, status.Error(codes.Internal, fmt.Sprintf("filesystem %v not mounted on any node", scaleVol.VolBackendFs))
                }
...
func (cs *ScaleControllerServer) ControllerUnpublishVolume(ctx context.Context, req *csi.ControllerUnpublishVolumeRequest) (*csi.ControllerUnpublishVolumeResponse, error) {
	...
	glog.V(4).Infof("ControllerUnpublishVolume : SKIP_MOUNT_UNMOUNT is set to %s", skipMountUnmount)
	if skipMountUnmount == "yes" {
		glog.V(4).Infof("ControllerUnpublishVolume : Skipping Unmount as SKIP_MOUNT_UNMOUNT is set to yes")
	...
	if err != nil {
		glog.Errorf("ControllerUnpublishVolume : Error in getting filesystem Name for %s", filesystemID)
		return nil, status.Error(codes.Internal, fmt.Sprintf("ControllerUnpublishVolume : Error in getting filesystem Name for %s. Error [%v]", filesystemID, err))
	}
	...
	if err != nil {
		glog.Errorf("ControllerUnpublishVolume : Error in getting filesystem mount details for %s", primaryfsName)
